...
static int silk_add_empty_frames(CAPTURE_CBDATA_SP_T *pSp, 
  ...
  if(pSp->pStream->clockHz <= 0) {
    return -1;
  } if((numFrames = (tsDeltaHz * (1000 / SILK_FRAME_DURATION_MS) / pSp->pStream->clockHz)) <= 0) {
    return rc;
  }
  ...
  LOG(X_WARNING("Adding %d silent SILK frames to account for timestamp gap of %dHz (max %dHz) for %s, ssrc: 0x%x, ts: %u, seq:%u"),
  ...
  for(idxFrame = 0; idxFrame < numFrames; idxFrame++) {
    ...
    //
    // This actually creates an invalid 1 byte packet that should be checked by the SILK decoder
    //
    buf[0] = SILK_LOST_FRAME_DATA;
    pCbData->cbStoreData(pCbData->pCbDataArg, buf, SILK_LOST_FRAME_LEN);
...
static int capture_silk_parse(CAPTURE_CBDATA_SP_T *pSp,
                             CAPTURE_STORE_CBDATA_T *pCbData,
                             int queueFr,
...
int cbOnPkt_silk(void *pUserData, const COLLECT_STREAM_PKTDATA_T *pPkt) {
  ...
  VSX_DEBUG(
    //VSX_DEBUGLOG_TIME
    LOG(X_DEBUG("rtp-recv-silk len:%d seq:%u ts:%u(%.3f) ts0:%u(%.3f, dlta:%.3f), mrk:%d ssrc:0x%x "
                   "flags:0x%x pQ:0x%x"),
  ...
  if(len < 1) {
    LOG(X_ERROR("RTP Invalid SILK payload length: %u"), len);
    return -1;
  }
  ...
  if((pSp->spFlags & CAPTURE_SP_FLAG_STREAM_STARTED) && pSp->lastPktTs < pPkt->u.rtp.ts) {
    ...
    silk_add_empty_frames(pSp, &cbData, queueFr, pPkt, maxTsDeltaHz);
  }
  ...
  if(!(pPkt->flags & PKT_FLAGS_DTMF2833)) {
    rc = capture_silk_parse(pSp, &cbData, queueFr, pData, len);
  }
...
int streamadd_silk(CAPTURE_CBDATA_SP_T *pSp) {
  CAPTURE_FILTER_T *pFilter = (CAPTURE_FILTER_T *) pSp->pStream->pFilter;
  int rc = 0;
  ...
  if(pFilter->u_seqhdrs.aud.common.clockHz == 0) {
    ...
    LOG(X_WARNING("Using default input SILK clock rate %u"), pFilter->u_seqhdrs.aud.common.clockHz);
  }

  //pSp->pStream->clockHz = pFilter->u_seqhdrs.aud.common.clockHz;

  LOG(X_DEBUG("Setup SILK RTP input %uHz 1 channel"), pFilter->u_seqhdrs.aud.common.clockHz);
